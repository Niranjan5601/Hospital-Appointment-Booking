...
def book_appoiment_form_submission(request):
    # request.POST[first_name="arun","lastname="kumar]
    ...
    print(f"mobile_no is {mobile_no}")

    appoinment_date=request.POST.get('appoinment_date')
    print(f"appoinment_date is {appoinment_date}")
    ...
    if book_appoinment_table.objects.filter(email_id=request.POST.get('email_id'),appoinment_date=request.POST.get('appoinment_date')):
        ...
        print("***appoinment adata saved succcessfully****")
        #mail code
        try: 
            subject = 'APPOINMENT BOOKING SUCCESSFULLY'
            message = f'Hi {first_name} {last_name},\nYour appoinment booking successfuly,\nbooking date and time:{booking_datetime}'
...
def login_form_submission(request):
    ...
    if admin_table.objects.filter(username=user_username,password=user_password):
        print("***login success*****")
        logger_data=admin_table.objects.get(username=user_username,password=user_password)
